...
' Inserts the given image at cell H57 on every worksheet, sized
' to a fixed 0.5-inch height with aspect ratio locked, white set
' as the transparent color, then nudged down 9pt and right 2pt
...
Private Sub InsertSignatureToAllSheets(picPath As String)
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        Set pic = ws.Shapes.AddPicture(picPath, msoFalse, msoCTrue, anchor.Left, anchor.Top, -1, -1)
...
' Fully locks every worksheet with the given password: no cell
