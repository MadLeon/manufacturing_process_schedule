...
'   - Same generic wrapper as mp/mod_SQLite.bas, adapted for dir_signature: the DIR workbook is
'     copied to many different customer folders on the network (see dir log/mod_CheckSelectedCell.bas),
'     so the SQLite DLLs cannot be found next to ThisWorkbook.Path like the other projects do. They
'     must instead be loaded from the fixed template folder (see DLL_DIR in Function_DatabaseSync.bas).
' -------------------------------------------------------------------------------------------------
...
Public Function InitializeSQLite(dbPath As String, dllDir As String) As Boolean
    ...
    Dim result As Long
    result = SQLite3Initialize(dllDir)
    If result <> SQLITE_INIT_OK Then
        Debug.Print "SQLite3 initialization failed. Please check if SQLite3.dll and SQLite3_StdCall.dll are in " & dllDir
    ...
    If result <> SQLITE_OK Then
        Debug.Print "Unable to open database " & dbPath & ". Please check if the file exists and permissions."
        SQLite3Free
        InitializeSQLite = False
        Exit Function
    End If
